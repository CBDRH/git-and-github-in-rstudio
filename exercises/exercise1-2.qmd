...
1. In RStudio, open the file `code/data-cleaning-1.R`
2. Update the details in the title box. You can edit this to include what you like. Standard items to list include: 
    ...
    - The input file (here `raw-data/national-health-survey-2022-table-2.xlsx`)
    - Any outputs that the code generates (here the output file will be the cleaned data `clean-data/vis1.Rda`)
    - Code author names
...
2. In the git panel, tick the box next to `code/data-cleaning-1.R` @Aarathy
...
download.file(url = myURL,
              destfile = 'raw-data/national-health-survey-2022-table-2.xlsx')
```
...
## Checkpoint
If you have run this code successfully, the file `national-health-survey-2022-table-2.xlsx` should be saved in your `raw-data` folder. Go ahead and open it---you can do so from within R Studio by going to the **Files** tab, clicking on the file name and selecting **View File**. @Aarathy
...
df1Raw <- read_excel(
            path = 'raw-data/national-health-survey-2022-table-2.xlsx',
...
Add the following blocks of code to your script to save the `df1Clean` file in your `clean-data` folder.
